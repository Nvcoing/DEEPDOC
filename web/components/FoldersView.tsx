...
const FoldersView: React.FC<FoldersViewProps> = ({
  t, folders, documents, currentFolderId, onNavigate, onCreateFolder, onUpload, onFilePreview, onDeleteFolder, onChatWithFolder
}) => {
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <header className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b border-slate-100 dark:border-slate-800 pb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="flex items-center gap-1 text-[10px] font-black uppercase text-slate-400 tracking-widest mt-1">
              <span>ROOT</span>
              {currentFolder && (
                <>
                  <ChevronRight className="w-3 h-3" />
                  <span className="text-indigo-500">{currentFolder.name}</span>
                </>
              )}
            </div>
          </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="relative group mr-2">
            ...
            <input 
              ...
              className="pl-10 pr-10 py-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-xs font-bold focus:ring-2 focus:ring-indigo-500 outline-none transition-all dark:text-white shadow-sm w-48 md:w-64"
            />
            {searchQuery && (
              <button 
          ...
          <button 
            ...
            className="flex items-center gap-2 px-4 py-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl font-bold text-xs hover:bg-slate-50 transition-all dark:text-white"
          >
            <FolderPlus className="w-4 h-4 text-indigo-500" /> {t.createFolder}
          </button>
          
          <label className="flex items-center gap-2 px-4 py-2.5 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-slate-700 rounded-xl font-bold text-xs hover:bg-slate-200 transition-all cursor-pointer">
      ...
      {isCreating && (
        <form onSubmit={handleCreate} className="bg-white dark:bg-slate-900 border-2 border-indigo-500 p-4 rounded-2xl flex items-center gap-3 animate-in zoom-in-95 duration-200 shadow-lg shadow-indigo-100 dark:shadow-none">
          ...
          />
          <button type="button" onClick={() => setIsCreating(false)} className="text-xs font-black text-slate-400 uppercase hover:text-slate-600 transition-colors">{t.back}</button>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        ...
        {filteredFolderDocs.map(doc => (
          <div 
            ...
            className="group bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4 rounded-2xl hover:shadow-lg hover:border-blue-400 transition-all cursor-pointer"
          >
            <div className="space-y-3">
              <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${doc.type === 'pptx' ? 'bg-orange-50 text-orange-600' : 'bg-blue-50 text-blue-600'}`}>
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                   ...
                   <span className={`w-1.5 h-1.5 rounded-full ${doc.status === 'approved' ? 'bg-green-500' : 'bg-orange-400'}`} title={doc.status === 'approved' ? t.approved : t.pending} />
                </div>
              </div>
            </div>
          </div>
        ...
        {!hasContent && !isCreating && (
          <div className="col-span-full py-20 text-center space-y-4">
            <div className="w-20 h-20 bg-slate-100 dark:bg-slate-800 rounded-full flex items-center justify-center mx-auto animate-pulse">
              {searchQuery ? <Search className="w-10 h-10 text-slate-300" /> : <Folder className="w-10 h-10 text-slate-300" />}
            </div>
            <p className="text-slate-400 font-black uppercase text-xs tracking-widest">
              {searchQuery ? `Không tìm thấy kết quả cho "${searchQuery}"` : t.emptyFolder}
            </p>
          </div>
        )}
      </div>
