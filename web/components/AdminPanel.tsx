  ...
  return (
    <div className="p-8 md:p-12 max-w-7xl mx-auto space-y-12 animate-in fade-in duration-700">
      ...
      <div className="min-h-[500px]">
              ...
              {pendingDocs.length > 0 ? pendingDocs.map(doc => (
                <div key={doc.id} className="bg-white dark:bg-slate-900 border border-slate-200/60 p-6 rounded-[2rem] flex flex-col sm:flex-row sm:items-center justify-between shadow-sm gap-6 border-b-4 border-b-slate-100 dark:border-b-slate-950">
                  ...
                  <div className="flex items-center gap-4">
                    <button onClick={() => onDeletePermanently(doc.id)} className="p-3.5 text-red-500 rounded-xl hover:bg-red-50 transition-colors" title={t.reject}>
                    ...
                    <button onClick={() => onApprove(doc.id, 'approved')} className="px-10 py-4 bg-green-500 text-white rounded-2xl shadow-lg shadow-green-100 dark:shadow-none font-black text-xs uppercase tracking-widest hover:bg-green-600 transition-all flex items-center gap-2">
                     ...
                     <p className="text-[11px] font-black uppercase tracking-[0.5em] italic">{t.noActivities}</p>
                   </div>}
            </div>
          </section>
        )}
        ...
        {activeTab === 'depts' && !selectedDeptId && (
          <section className="space-y-10 animate-in slide-in-from-bottom-4 duration-500">
            <div className="flex items-center justify-between">
               <h3 className="font-black text-2xl dark:text-white flex items-center gap-3 italic uppercase"><div className="w-2 h-8 bg-indigo-500 rounded-full" /> {t.orgStructure}</h3>
               <button onClick={() => setIsAddingDept(true)} className="flex items-center gap-2.5 px-6 py-3.5 bg-indigo-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:scale-105 shadow-xl shadow-indigo-100 transition-all"><Plus className="w-4 h-4" /> {t.create}</button>
            </div>
            ...
            {isAddingDept && (
              <div className="bg-white dark:bg-slate-900 border-2 border-indigo-500 p-10 rounded-[2.5rem] shadow-2xl max-w-lg mx-auto mb-12 relative">
                ...
                <h4 className="font-black text-xl uppercase italic mb-8">{t.newDept}</h4>
                <input autoFocus type="text" placeholder={t.deptNamePlaceholder} className="w-full bg-slate-50 dark:bg-slate-800 border-none px-6 py-5 rounded-2xl text-sm font-bold shadow-inner focus:ring-2 focus:ring-indigo-500 outline-none" value={newDeptName} onChange={e => setNewDeptName(e.target.value)} />
                <button onClick={handleAddDept} className="w-full mt-6 py-5 bg-indigo-600 text-white rounded-[1.8rem] font-black uppercase text-xs tracking-[0.2em] shadow-lg shadow-indigo-100 hover:bg-indigo-700 transition-all">{t.saveStructure}</button>
              </div>
            )}
              ...
              {departments.map(dept => (
                <div key={dept.id} onClick={() => { setSelectedDeptId(dept.id); setSelectedSubFolderId(null); }} className="group bg-white dark:bg-slate-900 border border-slate-200 p-8 rounded-[2.5rem] flex flex-col gap-8 hover:border-indigo-400 hover:shadow-2xl cursor-pointer relative overflow-hidden transition-all border-b-4 border-b-slate-100 dark:border-b-slate-950">
                  <div className="absolute top-6 right-6 opacity-0 group-hover:opacity-100 flex gap-2 transition-opacity">
                    <div onClick={(e) => { e.stopPropagation(); if (window.confirm(t.confirmDelete)) setDepartments(departments.filter(d => d.id !== dept.id)); }} className="p-2.5 bg-red-50 text-red-500 rounded-xl hover:bg-red-100"><Trash2 className="w-4 h-4" /></div>
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-400 font-black uppercase mt-2 tracking-widest">{users.filter(u => u.departmentId === dept.id).length} {t.members}</p>
                  </div>
                </div>
              ))}
            </div>
               ...
               <div className={`${selectedSubFolderId ? 'lg:col-span-12' : 'lg:col-span-8'} space-y-8`}>
                  <div className="flex items-center justify-between">
                    <h3 className="font-black text-xl italic uppercase dark:text-white flex items-center gap-3">
                      ...
                      {selectedSubFolderId ? truncateWords(currentSubFolder?.name || "", 4) : t.mainStorage}
                    </h3>
                    <div className="flex items-center gap-3">
                      ...
                      <label className={`flex items-center gap-2 px-6 py-3 bg-indigo-600 text-white rounded-2xl font-black text-[11px] uppercase tracking-widest cursor-pointer shadow-lg hover:bg-indigo-700 transition-all ${isUploading ? 'opacity-50' : ''}`}>
                        {isUploading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Upload className="w-4 h-4" />} {isUploading ? t.processing.toUpperCase() : t.uploadDesc.toUpperCase()}
                  ...
                  {isCreatingFolder && (
                    <form onSubmit={handleCreateDeptFolder} className="bg-white dark:bg-slate-900 border-2 border-indigo-500 p-5 rounded-2xl flex items-center gap-4 animate-in zoom-in-95 shadow-2xl">
                      ...
                      <input autoFocus type="text" placeholder={t.folderNamePlaceholder} className="flex-1 bg-transparent border-none outline-none font-black text-sm dark:text-white italic" value={newFolderName} onChange={(e) => setNewFolderName(e.target.value)} />
                      <button type="button" onClick={() => setIsCreatingFolder(false)} className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t.cancel}</button>
                      <button type="submit" className="px-6 py-2 bg-indigo-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest">{t.create}</button>
                    </form>
                  )}
                     ...
                     {currentLevelDocs.map(doc => (
                       <div key={doc.id} className={`group bg-white dark:bg-slate-900 border border-slate-200 p-6 rounded-[2rem] flex items-center gap-5 shadow-sm hover:shadow-xl transition-all relative border-b-4 border-b-slate-50 dark:border-b-slate-950 ${doc.status === 'uploading' ? 'opacity-70 animate-pulse' : ''}`}>
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-[9px] font-black uppercase text-slate-400 tracking-widest mt-1">{doc.status === 'uploading' ? t.processing : `${doc.type} • ${doc.uploadDate}`}</p>
                          </div>
                          {doc.status !== 'uploading' && (
                            <button onClick={() => { if(window.confirm(t.confirmDelete)) setDocuments(allDocs.filter(d => d.id !== doc.id)); }} className="p-2 opacity-0 group-hover:opacity-100 hover:bg-red-50 text-red-500 rounded-xl transition-all"><Trash2 className="w-4 h-4" /></button>
                          )}
                       </div>
                     ))}
                  </div>
        ...
        {activeTab === 'users' && (
          <section className="space-y-10 animate-in slide-in-from-bottom-4 duration-500">
             <div className="flex items-center justify-between"><h3 className="font-black text-2xl dark:text-white italic uppercase flex items-center gap-3"><div className="w-2 h-8 bg-indigo-500 rounded-full" /> {t.userDirectory}</h3><button onClick={() => setIsAddingUserGlobal(true)} className="flex items-center gap-2.5 px-6 py-3.5 bg-indigo-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl transition-all hover:scale-105"><UserPlus className="w-4 h-4" /> {t.addUser}</button></div>
             {isAddingUserGlobal && (
                <div className="bg-white dark:bg-slate-900 border-2 border-indigo-500 p-12 rounded-[3rem] shadow-2xl max-w-2xl mx-auto mb-12 relative animate-in zoom-in-95">
                  ...
                  <h4 className="font-black text-2xl uppercase italic mb-10 text-center">{t.addUser}</h4>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <input type="text" placeholder={t.namePlaceholder || "Họ và tên..."} className="bg-slate-50 dark:bg-slate-800 p-5 rounded-2xl font-bold shadow-inner" value={newUser.name} onChange={e => setNewUser({...newUser, name: e.target.value})} />
                    <input type="email" placeholder={t.identifyEmail} className="bg-slate-50 dark:bg-slate-800 p-5 rounded-2xl font-bold shadow-inner" value={newUser.email} onChange={e => setNewUser({...newUser, email: e.target.value})} />
                    <input type="password" placeholder={t.passwordPlaceholder} className="bg-slate-50 dark:bg-slate-800 p-5 rounded-2xl font-bold shadow-inner" value={newUser.password} onChange={e => setNewUser({...newUser, password: e.target.value})} />
                    ...
                    <button onClick={() => { if(newUser.name) { setUsers([...users, { ...newUser, id: `u-${Date.now()}`, allowedDocIds: [] }]); setIsAddingUserGlobal(false); } }} className="md:col-span-2 py-5 bg-indigo-600 text-white rounded-[1.8rem] font-black uppercase tracking-widest text-xs shadow-xl hover:bg-indigo-700 mt-4 transition-all">{t.authAccount}</button>
                  </div>
                </div>
             )}
             <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {users.map(user => (
                  <div key={user.id} className="bg-white dark:bg-slate-900 border border-slate-200 p-8 rounded-[2.5rem] flex items-center justify-between hover:border-indigo-400 hover:shadow-2xl transition-all border-b-4 border-b-slate-100 dark:border-b-slate-950">
                    ...
                    <span className={`px-5 py-2 rounded-full text-[10px] font-black uppercase tracking-widest ${user.role === 'admin' ? 'bg-purple-100 text-purple-700' : 'bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-400'}`}>{user.role === 'admin' ? t.adminRole : t.employee}</span>
                  </div>
                ))}
             </div>
          </section>
